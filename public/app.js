...
function updateDisplay(data, stats) {
    ...
    document.getElementById('gasLevel').textContent = (data.gasLevel || 0).toFixed(0);
    document.getElementById('lightLevel').textContent = (data.lightLevel || 0).toFixed(1);
    
    // تحديث حالات القيم
    updateTempStatus(data.temperature);
    updateHumStatus(data.humidity);
    updateGasStatus(data.gasLevel || 0);
...
function updateGasStatus(gas) {
    ...
    if (gas < 500) {
        statusEl.textContent = 'نظيف ✅';
        statusEl.className = 'status-badge excellent';
    } else if (gas < 1500) {
        statusEl.textContent = 'طبيعي 👍';
        statusEl.className = 'status-badge good';
    } else if (gas < 3000) {
        statusEl.textContent = 'مرتفع ⚠️';
        statusEl.className = 'status-badge warning';
    } else {
        statusEl.textContent = 'خطير 🚨';
        statusEl.className = 'status-badge danger';
    }
}
...
function updateLightStatus(light) {
    ...
    if (light < 10) {
        statusEl.textContent = 'مظلم 🌑';
        statusEl.className = 'status-badge warning';
    } else if (light < 100) {
        statusEl.textContent = 'خافت 🌘';
        statusEl.className = 'status-badge good';
    } else if (light < 1000) {
        statusEl.textContent = 'مناسب ✅';
        statusEl.className = 'status-badge excellent';
    } else {
        statusEl.textContent = 'مشرق ☀️';
        statusEl.className = 'status-badge warning';
    }
}

function updateOverallStatus(temp, hum, gas, light) {
    const statusEl = document.getElementById('overallStatus');
    const iconEl = document.getElementById('statusIcon');
    const titleEl = document.getElementById('statusTitle');
    const messageEl = document.getElementById('statusMessage');
    
    const tempGood = (temp >= 18 && temp <= 26);
    const humGood = (hum >= 30 && hum <= 60);
    const gasGood = (gas < 1500);
    const lightGood = (light >= 10 && light < 1000);
    
    if (tempGood && humGood && gasGood && lightGood) {
        statusEl.className = 'overall-status excellent';
        iconEl.textContent = '✅';
        titleEl.textContent = 'البيئة ممتازة!';
        messageEl.textContent = 'جميع العوامل البيئية في المستوى المثالي';
    } else if (gas > 3000) {
        statusEl.className = 'overall-status poor';
        iconEl.textContent = '🚨';
        titleEl.textContent = 'خطر! مستوى الغاز مرتفع';
        messageEl.textContent = 'تحقق من تسرب الغاز وتحسين التهوية فوراً';
    } else if (temp > 30 || hum > 80 || gas > 1500) {
        statusEl.className = 'overall-status poor';
        iconEl.textContent = '❌';
